...
Only valid w.r.t. subtypes generated by @version
"""
...
macro version(id, required_field_statements)
    ...
    if !allunique(f.name for f in required_field_infos)
        msg = string("cannot have duplicate field names in `@schema` declaration; recieved: ", [f.name for f in required_field_infos])
        return :(throw(SchemaVersionDeclarationError($msg)))
    end
    ...
    isnothing(type_prefix) && throw(SchemaVersionDeclarationError("TODO"))
